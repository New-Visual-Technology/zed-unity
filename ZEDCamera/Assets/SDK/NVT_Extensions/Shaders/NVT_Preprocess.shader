Shader "NVT/Preprocess GreenScreen"
{
    Properties
    {
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            sampler2D _TmpWorkpieceMaskTex;
            sampler2D _MainTex;
            float4 _MainTex_ST;
            ...
            fixed4 frag(v2f i) : SV_Target
            {
                ...
                float alpha = tex2D(_TmpWorkpieceMaskTex, uv).r;
                ...
                if (_erosion >= 1) {
                    ...
                    float a1 = pow(saturate(tex2D(_TmpWorkpieceMaskTex, uv1).r / _smoothness), 1.5);
                    float a2 = pow(saturate(tex2D(_TmpWorkpieceMaskTex, uv3).r / _smoothness), 1.5);
                    float a3 = pow(saturate(tex2D(_TmpWorkpieceMaskTex, uv5).r / _smoothness), 1.5);
                    float a4 = pow(saturate(tex2D(_TmpWorkpieceMaskTex, uv7).r / _smoothness), 1.5);
